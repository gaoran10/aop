...
public class AmqpConsumerOriginal extends AmqpConsumer {
    ...
    public AmqpConsumerOriginal(QueueContainer queueContainer, Subscription subscription,
        ...
        super(queueContainer, subscription, subType, topicName, consumerId, priorityLevel, consumerName, isDurable,
                cnx, appId, metadata, readCompacted, subscriptionInitialPosition, keySharedMeta, channel, consumerTag,
                queueName, autoAck, queueService);
    }
    ...
    public Future<Void> sendMessages(List<Entry> entries, EntryBatchSizes batchSizes,
        ...
        connection.ctx.channel().eventLoop().execute(() -> {
            ...
            final double writeCount = count;
            asyncGetQueue().thenAccept(amqpQueue -> {
                ((PersistentQueue) amqpQueue).getQueueMetrics().readInc(writeCount);
            });
            batchSizes.recyle();
        });
        return writePromise;
