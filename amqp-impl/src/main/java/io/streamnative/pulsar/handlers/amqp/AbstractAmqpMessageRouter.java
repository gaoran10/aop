...
public abstract class AbstractAmqpMessageRouter implements AmqpMessageRouter {
    ...
    @Override
    public CompletableFuture<Void> routingMessageToEx(Entry entry, String routingKey, Map<String, Object> properties) {
        CompletableFuture<Void> future = new CompletableFuture<>();
        if (isMatch(properties)) {
            return destinationEx.writeMessageAsync(MessageConvertUtils.entryToMessage(entry, properties), routingKey)
                    .thenApply(__ -> null);
//            try {
